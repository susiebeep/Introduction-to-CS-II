 ...
 ** Description:  This program contains the function definition for an input validation
		  ...
		  If a non-integer is found in the input string, the user will be asked to
		  re-enter an integer. The cin input buffer will be cleared after each
		  incorrect input with cin.clear() and cin.ignore() is used to ignore
		  the rest of the input (up to 256 characters) that comes after the first
		  invalid character and skips to the next line. 

		  Once the an integer is returned to the calling program, any extraneous
		  input is removed from the input buffer with cin.ignore().
 ** Citations:	  Checking cin input stream produces an integer, Stack Overflow
		  ...
		  istream::ignore, C++ Reference, Cplusplus.com
...
int int_input_val(std::string input)
{
	...
	//the if statement is executed and the user is prompted to enter an integer
	//value. The while condition causes the do loop to iterate until there is valid
	//input from the user
